...
angular.module('mufcg').config(['$stateProvider', '$urlRouterProvider', '$locationProvider', function ($stateProvider, $urlRouterProvider, $locationProvider) {
...
angular.module('mufcg').service('authService', function () {
...
angular.module('mufcg').controller('HomeCtrl', ['$scope', 'NgMap', 'mapHelper', 'LOCATIONS', 'authService', 'Request', function ($scope, NgMap, mapHelper, LOCATIONS, authService, Request) {
    ...
    function loadRequests() {
        Request.getByAuthor(authService.getCurrentUser()._id).then(function (res) {
...
angular.module('mufcg').controller('LoginCtrl', ['$scope', 'User', 'authService', 'messagebox', '$location', function ($scope, User, authService, messagebox, $location) {
    $scope.auth = function () {
...
angular.module('mufcg').controller('RegisterCtrl', ['$scope', '$location', 'User', function ($scope, $location, User) {
    $scope.register = function () {
        if ($scope.user.password !== $scope.confirmPassword) alert('Senhas divergem!');else User.register($scope.user.name, $scope.user.identification, $scope.user.password, "student").then(function (data) {
...
angular.module("mufcg").factory('User', ['$http', 'PROPERTIES', function UserFactory($http, PROPERTIES) {
    return {
        register: function register(name, registration, password, role) {
            var params = {
                name: name,
                password: password,
        ...
        auth: function auth(registration, password) {
            var params = {
                registration: registration,
                password: password
            };
